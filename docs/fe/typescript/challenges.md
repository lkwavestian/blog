...
另外，还需要注意下错误的测试用例，当联合类型`K`中的某一个类型不是`T`的属性值时，需要报错，所以我们必须限制`K`对应的这一系列属性在`T`中存在，需要使用`extends` 和`keyof`
...
首先是`as const`，它是一种类型断言，用于将一个表达式（如数组、对象或字面量）视为常量值，从而推导出更具体的类型，它有以下几个特点：
...
但是这样写之后，错误测试用例没有触发，还需要限制一下参数的类型，不能为数组以及对象，只能为`string`、`number`、`symbol`这三种可以当作键值的类型
